...
**Task** Run the following code and describe the results. **Answer: I believe that the bigrams aren't telling us nothing. If we observe the percentages, we can see that they are repeated very little and also the most common bigrams are always very common in medical language.**
...
**Task** Run the following code and describe the results. **Answer: Percentages are very small too. I thing that comparing those to years (or more) , we could know which bigrams are common and we could discard them.**
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer: Perhaps we should say "abort"? It doesn't have to be negative. Similarly, "abundant" is not always positive.**
