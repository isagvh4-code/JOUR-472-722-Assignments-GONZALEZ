...
**Task** Write and run code to answer our original question: which zip code has gotten the most amount of money from the Maryland Tourism Board? Where is that zip code, and does that zip code make sense to you as the top recipient? Write a sentence describing the top result. **Answer 20001, it is in D.C. For me it doesn't has sense because the Zip Code is not even in Maryland. The total amount received by that ZC i 2.2 billion dollars. This is 1.3 billion more than in the second zip code that received the most money, and that one is indeed in Maryland.**

```{r}
granrecipient <- cleaned_md_grants_loans |>
  filter(str_detect(source, "MARYLAND")) |>
  group_by(zip5) |>
  summarize(total_money = sum(amount)) |>
