...
maryland_zcta <- read_csv('data/maryland_zcta.csv') |> clean_names() |>
  mutate(zcta5n=as.chracter(zcta5n)) 

```

```{r}
#join the Baltimore-related data
maryland_and_baltimore <- baltimore |> left_join(maryland_zcta, join_by(zip_code==zcta5n))
```
...
county_combined |>
  ...
  arrange(month, desc(total_calls))
print(months_calls)
...
```{r}
mostcalls <- county_combined |>
    group_by(county, location) |>
summarise(total_calls = n())
```

A2. **The only thing I see is that as the end of the year, or the colder months, approaches, the number of 911 calls decreases. \*I used IA to write the first chunk.**
...
```{r}
high_child_zip_calls <- maryland_and_baltimore |>
...
```{r}
highest_rate_zip_details <- maryland_and_baltimore |>
  distinct(district)
...
```{r}
overdose_counts_by_white <- maryland_and_baltimore |>
  group_by(pct_white_non_hispanic) |>
   summarize(total_calls = n()) |>
arrange(desc(pct_white_non_hispanic))
```
