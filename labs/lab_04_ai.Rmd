...
```{r}
maggie_story <- axolotr::ask(
  prompt = paste("Given the article about the death of Dame Maggie Smith, extract the names of films and TV shows mentioned in this article into a CSV file with the following structure, including headers, with no yapping, no backlashes: title, year,role, director, co-stars, awards", text),
  model = "llama-3.1-8b-instant"
)

maggie_story <- read_csv(maggie_story)
```
...
text= "r_code_example = 'umd_courses |>
