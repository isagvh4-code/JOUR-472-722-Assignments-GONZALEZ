...
**A2: There are 9,139 records. The database records the earthquakes of the last month in the world. We can know which place has the biggest earthquakes, and which one the smallest.**
...
```{r}
deepest_eq <- earthquakes|> 
   arrange(desc(depth))
  
```

**A3: The deepest earthquake was 643 km depth and its magnitude was 4.2 magnitude.**
...
**A7: The column is place. And 2360 earthquakes took place in Alaska.**
...
**A9: The average depth of the earthquakes type is 18.39. The negative numbers.**
...
```{r}
earthquakes_time <- earthquakes |>
  mutate(hour = hour(time))
```

```{r}
mostcommon <- earthquakes_time |>
  count(hour)|> 
  arrange(desc(n))
```

**A10: The hour were most earthquakes happen is 9.**
...
**A12**: **The hour with most serious earthquakes is 20 and the hour with less serious earthquakes is 13.**

------------------------------------------------------------------------

#### EXTRA Q What other questions are you interested in and how would you ask them in R?

**Which is the place with more magnitude 6 or more earthquakes?**

```{r}
placewithbigeq <-big_eq |>
  filter(mag >= 6) |>
  group_by(place) |>
  summarise(count = n()) |>
  arrange(desc(count))
...
**What's the smallest earthquake?**
