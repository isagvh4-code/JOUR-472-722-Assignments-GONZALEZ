...
**Q1.** How many departments have offered the courses with "Climate" in the title and at least one seat? Which department has offered the most? **A1. 32 departments. Atmospheric and Oceanic Science with 93.**
...
**Q2.** Of the departments listed in A1, which one is the most interesting to you, and why? **A2. Why does University Honors has climate related courses?**
...
**A3. Business and Managment with 2498 courses, School of Music with 2064, English with 2054, Communication with 1780 and Kinesiology with 1696**

```{r}
fall_courses2024 <- umd_courses |> filter(term == 202408)

head(fall_courses2024)
```

```{r}
selected_fall_courses2024 <- fall_courses2024 |> select(department,term, syllabus_count) |> filter(syllabus_count > 0)
                                                                                    head(selected_fall_courses2024)           
```

```{r}
selected_fall_courses2024 <- umd_courses |>
 group_by(department) |>
summarise(count=n())|>
  arrange(desc(count))
```
