...
library(ggplot2)
library(ggrepel)
census_api_key("fa67f547995f07083d6506ed18786d76a6fd1b30", install = TRUE)
```
...
**A1. December. // In PG, the months with the most overdose calls are the coldest months, whereas in Baltimore, they are the warmest months.**

```{r}
pg_calls_permonth <-pg_calls |> group_by(month) |> summarize(total_calls = n()) |> 
...
pg_calls_permonth |>
  ...
  labs(
    title="PGs Overdose Calls Per Month",
    x = "month",
    y = "total calls",
...
**A2. The last week of the year is the week with less calls. The week with more calls is one between the months of June and July. Most of the weeks the range is more or less 20 calls.**

```{r}
pg_callsperweek <-pg_calls |> group_by(week) |> summarize(total_calls = n())
...
**A3. I think that the scatterplot shows that lower the income of the zip code, more 911 overdose calls. This possibly means that the fewer economic resources there are, the more people suffer overdoses.**
...
joined_data <- pg_calls_zip |>
  inner_join(md_median_income_zip, by = c("zip_code" = "GEOID"))

ggplot(data = joined_data, aes(x = estimate, y = total_calls)) +
  ...
  labs(
    title = "911 Calls vs. Median Household Income by Zip Code (MD)",
    subtitle = "Source: ACS 2019 5-year Estimates and 911 Call Data",
    x = "Median Household Income (USD)",
    y = "Total 911 Calls",
    caption = "Data joined by ZCTA/Zip Code"
  ) +
