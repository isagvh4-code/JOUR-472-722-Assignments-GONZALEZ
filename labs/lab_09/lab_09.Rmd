...
**A1.** <https://datawrapper.dwcdn.net/6I0vf/1/>

I chose this type of bars to keep it simple and easy to understand. Also I only used the columns of the percentages to use the smaller numbers, the percentage versus the total numbers.

```{r}
md_gov_county <- read_csv("data/md_gov_county.csv")
md_gov_county_2018 <- read_csv("data/md_gov_county_18.csv")

md_2022 <- md_gov_county |>
  select('GEOID', 'County', 'Cox')

md_2018 <- md_gov_county_2018 |>
  select('GEOID', 'County', 'Hogan')

md_both <-md_2018 |>
  inner_join(md_2022,
             by= c("County", "GEOID"))

md_both_final <- md_both |>
  mutate(
    pctHogan = (Hogan / 2304512) * 100,
    pctCox = (Cox /1754475) * 100
  )

write_csv(md_both_final, "md_final.csv")
```
...
**A2.** <https://datawrapper.dwcdn.net/Lc5um/1/>[\
](#0)I think that the story might be that the early vote and vote by mail is increasing in Maryland after the Covid-19 pandemic.

```{r}
md_votes_by_type <- read_csv("data/md_votes_by_type.csv")

md_type_pct <- md_votes_by_type |>
  mutate(
    pctEarly = (Early / Total) * 100,
    pct_Election_Day = (`Election Day` / Total) * 100,
    pct_Mail = (Mail / Total) * 100
  )

md_type_cleaned <- md_type_pct|>
  select(Year, pctEarly, pct_Election_Day, pct_Mail)

write_csv(md_type_cleaned, "md_type.csv")
```
...
**A3.** <https://datawrapper.dwcdn.net/RFtdH/1/>

I wasn't able to round the percentage in DataWrapper so I didn't that number because it looked horrible.

```{r}
overdose_calls <- read_csv("data/montgomery_2022_overdoses.csv")

overdose_calls_zip <- overdose_calls|>
  group_by(zip) |>
  summarise(
    num_calls = n() 
  ) |>
  ungroup()|>
    mutate(
    total = sum(num_calls),
    pct = (num_calls / total) * 100
  ) |>
  select(zip, num_calls, pct) |>
    arrange(desc(num_calls))

write_csv(overdose_calls_zip, "montgomerycalls.csv")
```
